...
This module implements a Code Property Graph parser for C#,
...
class CSharpCPGParser(CPGParser):
    """C# CPG parser using regex patterns."""
    ...
    def __init__(self):
        ...
        # C# patterns
        self.patterns = {
            'using': r'using\s+(?:static\s+)?([\w\.]+);',
            'namespace': r'namespace\s+([\w\.]+)\s*\{',
            'class': r'(?:public\s+|private\s+|protected\s+|internal\s+)?(?:abstract\s+|sealed\s+|static\s+)?class\s+(\w+)(?:\s*:\s*([\w\s,<>]+))?\s*\{',
            'interface': r'(?:public\s+|private\s+|protected\s+|internal\s+)?interface\s+(\w+)(?:\s*:\s*([\w\s,<>]+))?\s*\{',
            'struct': r'(?:public\s+|private\s+|protected\s+|internal\s+)?struct\s+(\w+)(?:\s*:\s*([\w\s,<>]+))?\s*\{',
            'enum': r'(?:public\s+|private\s+|protected\s+|internal\s+)?enum\s+(\w+)\s*\{',
            'method': r'(?:public\s+|private\s+|protected\s+|internal\s+)?(?:static\s+)?(?:virtual\s+|override\s+|abstract\s+)?(\w+(?:<[^>]+>)?)\s+(\w+)\s*\([^)]*\)\s*\{',
            'constructor': r'(?:public\s+|private\s+|protected\s+|internal\s+)?(\w+)\s*\([^)]*\)(?:\s*:\s*(?:base|this)\([^)]*\))?\s*\{',
            'property': r'(?:public\s+|private\s+|protected\s+|internal\s+)?(?:static\s+)?(\w+(?:<[^>]+>)?)\s+(\w+)\s*\{[^}]*\}',
            'field': r'(?:public\s+|private\s+|protected\s+|internal\s+)?(?:static\s+)?(?:readonly\s+)?(\w+(?:<[^>]+>)?)\s+(\w+)(?:\s*=\s*[^;]+)?;',
            'variable_decl': r'(\w+(?:<[^>]+>)?)\s+(\w+)(?:\s*=\s*[^;]+)?;',
            'assignment': r'(\w+(?:\.\w+|\[\w+\])*)\s*=\s*([^;]+);',
            'method_call': r'(\w+(?:\.\w+)*)\s*\([^)]*\)',
    ...
    def parse(self, content: str, file_path: str) -> CodePropertyGraph:
        """Parse C# source code and generate complete CPG."""
    ...
    def build_ast(self, content: str, file_path: str) -> CodePropertyGraph:
        """Build Abstract Syntax Tree for C# code."""
        ...
        self._process_usings(content, lines, module_id)
    ...
    def _process_usings(self, content: str, lines: List[str], parent_id: str):
        """Process using statements."""
        for match in re.finditer(self.patterns['using'], content, re.MULTILINE):
            using_name = match.group(1)
            start_line = content[:match.start()].count('\n') + 1
            
            using_node = self.create_node(
                NodeType.IMPORT,
                using_name,
                match.group(0),
                self.current_file_path,
            ...
            using_id = self.current_cpg.add_node(using_node)
            
            # Link to parent
            edge = self.create_edge(parent_id, using_id, EdgeType.AST_CHILD)
    ...
    def _process_classes(self, content: str, lines: List[str], parent_id: str):
        ...
        for match in re.finditer(self.patterns['class'], content, re.MULTILINE):
            ...
            inheritance = match.group(2) if match.groups() and len(match.groups()) > 1 else None
            ...
            # Get attributes
            attributes = self._get_attributes_before(content, match.start())
    ...
    def _process_structs(self, content: str, lines: List[str], parent_id: str):
        ...
        for match in re.finditer(self.patterns['struct'], content, re.MULTILINE):
            ...
            self._process_class_members(struct_content, struct_id, start_line, struct_name)
    ...
    def _process_namespace_members(self, namespace_content: str, namespace_id: str, 
        ...
        # Recursively process classes, interfaces, etc. within namespace
        self._process_classes(namespace_content, [], namespace_id)
        self._process_interfaces(namespace_content, [], namespace_id)
        self._process_structs(namespace_content, [], namespace_id)
        self._process_enums(namespace_content, [], namespace_id)
    
    def _process_class_members(self, class_content: str, class_id: str, 
                              class_start_line: int, class_name: str):
        """Process methods, properties, and fields within a class."""
        # Process methods
        for match in re.finditer(self.patterns['method'], class_content, re.MULTILINE):
            ...
            # Get attributes
            attributes = self._get_attributes_before(class_content, match.start())
        ...
        for match in re.finditer(self.patterns['property'], class_content, re.MULTILINE):
            ...
            property_node = self.create_node(
                NodeType.VARIABLE,
                property_name,
                match.group(0),
                self.current_file_path,
            ...
            property_id = self.current_cpg.add_node(property_node)
            
            # Track for DFG
            if property_name not in self.variable_definitions:
                self.variable_definitions[property_name] = []
            self.variable_definitions[property_name].append(property_id)
            
            # Link to class
            edge = self.create_edge(class_id, property_id, EdgeType.AST_CHILD)
            self.current_cpg.add_edge(edge)
    ...
    def _process_method_body(self, method_content: str, method_id: str, method_start_line: int):
        ...
        for match in re.finditer(self.patterns['assignment'], method_content, re.MULTILINE):
            ...
            used_vars = self._extract_csharp_variables_from_code(match.group(0))
    ...
    def _extract_csharp_variables_from_code(self, code: str) -> List[str]:
        ...
        csharp_keywords = {
            ...
            'break', 'continue', 'goto', 'try', 'catch', 'finally', 'throw', 'new',
            'this', 'base', 'virtual', 'override', 'abstract', 'sealed', 'partial',
            'async', 'await', 'var', 'void', 'int', 'string', 'bool', 'char', 'byte',
            'short', 'long', 'float', 'double', 'decimal', 'object', 'dynamic'
        }
        ...
        for match in re.finditer(r'\b([a-zA-Z_][a-zA-Z0-9_]*)\b', code):
            var_name = match.group(1)
            if var_name not in csharp_keywords and var_name not in variables:
    ...
    def _process_control_flow_in_method(self, method_content: str, method_id: str, 
        ...
        for pattern_name in ['for_loop', 'foreach_loop', 'while_loop']:
            for match in re.finditer(self.patterns[pattern_name], method_content, re.MULTILINE):
                ...
                loop_type = pattern_name.split('_')[0]  # 'for', 'foreach', or 'while'
    ...
    def _get_attributes_before(self, content: str, pos: int) -> List[str]:
        """Get attributes that appear before a given position."""
        attributes = []
        
        # Look backwards from the position to find attributes
        text_before = content[:pos]
        lines_before = text_before.split('\n')
        
        # Check the last few lines for attributes
        for line in reversed(lines_before[-5:]):  # Check last 5 lines
            ...
            if line.startswith('[') and line.endswith(']'):
                ...
                # Stop if we hit non-attribute, non-comment code
                break
